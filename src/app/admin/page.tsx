...
function EstadoSistema({ bolsa, fuenteDatos, onBolsaCorregida }: {
  bolsa: BolsaMinutos | null
  fuenteDatos: string
  onBolsaCorregida: () => void
}) {
  const [corrigiendoBolsa, setCorrigiendoBolsa] = useState(false)
  const [diagnostico, setDiagnostico] = useState<any>(null)
  const [msgBolsa, setMsgBolsa] = useState('')

  const diagnosticarBolsa = async () => {
    setCorrigiendoBolsa(true); setMsgBolsa(''); setDiagnostico(null)
    try {
      const res = await fetch('/api/fix-bolsa')
      const d = await res.json()
      setDiagnostico(d)
    } catch (e) { setMsgBolsa('Error al diagnosticar') }
    setCorrigiendoBolsa(false)
  }

  const corregirBolsa = async (saldo: number) => {
    setCorrigiendoBolsa(true)
    try {
      const res = await fetch('/api/fix-bolsa', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      const d = await res.json()
      if (d.ok) {
        setMsgBolsa(`✓ ${d.mensaje}`)
        setDiagnostico(null)
        onBolsaCorregida()
      } else {
        setMsgBolsa(`Error: ${d.error}`)
      }
    } catch (e) { setMsgBolsa('Error al corregir') }
    setCorrigiendoBolsa(false)
  }
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm space-y-4">
      ...
      <div className="border-t border-slate-100 pt-4">
        <div className="text-[10px] font-bold uppercase tracking-wider text-amber-700 mb-2">Corrección de Bolsa</div>
        <p className="text-xs text-slate-500 mb-3 leading-relaxed">
          Si el contador de minutos está incorrecto, diagnostica y corrige el saldo real automáticamente.
        </p>

        {!diagnostico ? (
          <button onClick={diagnosticarBolsa} disabled={corrigiendoBolsa}
            className="w-full flex items-center justify-center gap-2 py-2 rounded text-xs font-semibold border border-amber-300 text-amber-800 bg-amber-50 hover:bg-amber-100 disabled:opacity-40 transition-colors">
            {corrigiendoBolsa
          ...
          <div className="space-y-3">
            <div className="bg-slate-50 rounded-lg p-3 text-xs space-y-1.5">
              <div className="flex justify-between">
                <span className="text-slate-500">Saldo actual (en Blob)</span>
                <span className="font-bold text-red-700 tabular-nums">{fmt(diagnostico.saldoActualEnBlob)} min</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500">Saldo correcto</span>
                <span className="font-bold text-emerald-700 tabular-nums">{fmt(diagnostico.saldoCorrecto)} min</span>
              </div>
              <div className="flex justify-between border-t border-slate-200 pt-1.5">
                <span className="text-slate-500">Diferencia</span>
                <span className="font-bold text-amber-700 tabular-nums">
                  {fmt(Math.abs(diagnostico.diferencia || 0))} min {(diagnostico.diferencia || 0) > 0 ? 'de más' : 'de menos'}
                </span>
              </div>
            </div>
            {diagnostico.minutosRealesPorMes && (
              <div className="bg-slate-50 rounded-lg p-3 text-xs">
                <div className="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-2">Consumo real por mes</div>
                {Object.entries(diagnostico.minutosRealesPorMes).map(([mes, min]) => (
                  <div key={mes} className="flex justify-between py-0.5">
                    <span className="text-slate-600">{mes}</span>
                    <span className="tabular-nums font-semibold text-[#0F2444]">{fmt(min as number)} min</span>
                  </div>
                ))}
              </div>
            )}
            <div className="grid grid-cols-2 gap-2">
              <button onClick={() => setDiagnostico(null)}
                className="py-2 rounded text-xs font-semibold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-colors">
                Cancelar
              </button>
              <button onClick={() => corregirBolsa(diagnostico.saldoCorrecto)} disabled={corrigiendoBolsa}
                className="py-2 rounded text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-40 transition-colors">
                {corrigiendoBolsa ? 'Corrigiendo...' : `✓ Aplicar ${fmt(diagnostico.saldoCorrecto)} min`}
              </button>
            </div>
          </div>
        )}
        ...
        {msgBolsa && (
          <div className={`mt-2 text-xs p-2.5 rounded ${msgBolsa.startsWith('✓') ? 'bg-emerald-50 text-emerald-800 border border-emerald-200' : 'bg-red-50 text-red-800 border border-red-200'}`}>
            {msgBolsa}
          </div>
        )}
      </div>
      ...
      <main className="max-w-6xl mx-auto px-8 py-10">
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <EstadoSistema bolsa={bolsa} fuenteDatos={fuenteDatos} onBolsaCorregida={cargarEstado} />
